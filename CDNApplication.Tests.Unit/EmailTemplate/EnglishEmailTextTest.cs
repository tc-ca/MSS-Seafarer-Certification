namespace CDNApplication.Tests.Unit.EmailTemplate
{
    ...
    public class EnglishEmailTextTest
    {
        ...
        {
            return @"<head><style>body { font-family: Arial; line-height: 25px; }table, th, td { font-family: Arial; border: 1px solid black; border-collapse: collapse; padding: 10px; }.sig{ font-size: small; }</style></head><body><p><i>**Le texte français suit l’anglais**<br/>{English_Introduction}<h4>Voici un relevé de votre demande:</h4><table><tr><td>Numéro de candidat</td><td>{CDN_Number}</td></tr><tr><td>Numéro de téléphone</td><td>{Phone_Number}</td></tr><tr><td>Adresse électronique</td><td>{Email_Address}</td></tr><tr><td>Brevet sélectionné</td><td>{Selected_CertificateType}</td></tr><tr><td>Type de brevet</td><td>{Submission_Type_Fr}</td></tr><tr><td>Documents</td><td>{DOCUMENT}</td></tr></table>{English_Signature}<hr /><p><i>{French_Introduction}<h4>Here is a record of your submission:</h4><table><tr><td>Candidate Document Number</td><td>{CDN_Number}</td></tr><tr><td>Phone number</td><td>{Phone_Number}</td></tr><tr><td>Email address</td><td>{Email_Address}</td></tr><tr><td>Selected certificate</td><td>{Selected_CertificateType}</td></tr><tr><td>Type of certificate</td><td>{Submission_Type_En}</td></tr><tr><td>Documents</td><td>{DOCUMENT}</td></tr></table>{French_Signature}</body>";
        }
    }
}
